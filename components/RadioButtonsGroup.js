...
export default class RadioGroup extends Component {
  ...
  render() {
    if(!this.props.radioButtons || this.props.radioButtons.length == 0)
      return (
    ...
    const renderRadioButton = this.props.render;
    return (
      <View style={styles.container}>
...
class RadioButton extends Component {
  render() {
    ...
    return (
      <View
        style={[layout, { opacity, marginHorizontal: 10, marginVertical: 5, width: '100%'}]}
        >
      <TouchableOpacity
      style={{alignSelf:'center'}}
        ...
        {renderRadioButton ? (renderRadioButton(data)) : (<Text style={[{ alignSelf: 'center' }, margin, {color: data.labelColor}]}>{data.label}</Text>)}
      </View>
    );
  }
}
